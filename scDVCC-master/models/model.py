...
class model_Trainer(embedder):
    ...
    def _init_model(self):
        ...
        self.model = scDVCC(self.c_g_graph_knn.metadata(), self.adata.n_vars, self.args.n_clusters, layers,
                            self.args.tau, self.args.alpha, self.args.granularity, self.device).to(self.device)
        self.gene_embedding = nn.Embedding(self.adata.n_vars, layers[-1]).to(self.device)
...
class scDVCC(nn.Module):
    ...
    def predict_full_cell_rep(self, loader, gene_embedder):
        ...
        for batch in loader:
            ...
            gene_emb = gene_embedder.weight[batch['gene'].n_id]
            batch['gene'].x = gene_emb
